...
export function createCommandHandler({ consoleUI, scriptBasePath = "/public/scripts" }: CommandHandlerOptions) {
  const createSandboxConsole = () => ({
    log: (...args: any[]) => consoleUI.log(...args),
    info: (...args: any[]) => consoleUI.info(...args),
    warn: (...args: any[]) => consoleUI.warn(...args),
    error: (...args: any[]) => consoleUI.error(...args),
    clear: () => consoleUI.clear(),
  })

  const executeScript = async (code: string, label = "script") => {
    try {
  ...
  const resolveScriptUrl = (name: string): string => {
  ...
  const interpolateMathInTemplate = (text: string): string => {
  ...
  const runFromUrl = async (scriptName: string) => {
  ...
  const handleCommand = async (rawCommand: string) => {
    ...
    if (isMathExpression(commandText)) {
      try {
        ...
        consoleUI.error(`Math error: ${message}`)
      }
      return
    }
